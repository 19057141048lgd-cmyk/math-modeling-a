...
5 核时三问平均加速比分别为 **3.18 / 3.85 / 3.97**（朴素基线 2.72）。所有用例、所有问题的 makespan 随核数单调不增。

**全部 100 个用例**（只用模型选优，7.4 节）：5 核平均加速比 **3.76 / 4.32 / 4.43**；4 核问题2 平均 3.69，比“只做作业级分配”的理论上界 3.10（1.4 节）高 19%。1500 个组合的 makespan 全部随核数单调不增，spill 模型在 1500 个方案上与官方逐字节一致。8 进程约 1.5 小时跑完，时间几乎都花在报告成绩所需的官方评估上。
...
4. **官方单核基线本身可能被 spill 拖慢**。例如 case_044 单核额外换入换出 4.44 MB，L1 峰值打满；而细粒度子图能固定一个低工作集的执行顺序，spill 降为 0。这是出现“超线性加速”和“1 核加速比 > 1”的原因，完全在规则之内。这种情况并不少见，见 1.4 节的内存压力统计。
...
$$\min\ \operatorname{lex}\big(T_{\text{make}}(\sigma,\kappa,\pi),\ \Delta B,\ -h\big),\qquad
\bar S_N=\frac{1}{|\mathcal C|}\sum_{c\in\mathcal C}\frac{T^{(1)}_c}{T^{(N)}_c}$$

主目标是 makespan，次目标是新增搬运字节 $\Delta B$ 和 Cache 命中率 $h$。$T^{(1)}_c$ 是官方 `singlecore_evaluate.py` 给出的单核 makespan。
...
- **只用模型（`--verify 0`）**：取主场景候选和额外候选中 $\hat T$ 最小者，选优时不调用官方评估器；问题3 中场景 B 调度器生成的候选只在复核模式下参与。与复核结果的差距见 7.3 节，平均只高约 0.8%。报告成绩时选中方案本来就要官方评估一次，评估后再与两个兜底方案比较（`run_batch.fallback`），使结果同样随核数单调不增、只读 Cache 不劣于无 L2：
  - 上一核数的方案补空核后官方 makespan 不变，直接比较，不需要额外评估；
...
- 1 核时也有收益：相对无 L2 平均 1.027×（相对官方单核 1.071×），case_044 达到 1.43×，来源是 spill 回读命中。
...
| 问题1 | 1.014 | 1.785 | 2.429 | 2.791 | 3.182 |
| 问题2 | 1.043 | 2.054 | 2.893 | 3.494 | 3.849 |
| 问题3 | 1.071 | 2.063 | 2.931 | 3.581 | 3.971 |
...
完整的 1–5 核逐用例表见 `results/table_p1.md`、`table_p2.md`、`table_p3.md`（其中 `table_p3.md` 含 Cache 相对无 L2 的加速比和命中率），原始数据见 `results/summary.csv`，朴素基线见 `results/baseline.csv`。
...
|---|---|---|---|---|---|
| 问题1 | 1.007 | 1.757 | 2.401 | 2.758 | 3.150 |
| 问题2 | 1.043 | 2.044 | 2.844 | 3.476 | 3.828 |
| 问题3 | 1.071 | 2.052 | 2.896 | 3.560 | 3.932 |
...
- **1 核**：初版在 1 核时直接返回单子图方案。现在单子图方案也有预测值，和其它候选一起比较：问题2、问题3 的 1 核结果与复核完全一致，问题1 平均差距从 1.4% 降到 0.7%。
...
命令为 `python -m solver.run_batch --cases all --verify 0 --workers 8 --out results_all`，共 1500 个“用例 × 问题 × 核数”组合，全部成功。8 进程约 89 分钟跑完；之后按 3.1 节的粒度自适应重算了 3 个用例，约 10 分钟。逐用例表见 `results_all/table_p1.md`、`table_p2.md`、`table_p3.md`，原始数据见 `results_all/summary.csv`。其中 16 个代表用例的结果与 7.3 节现版本完全相同（240 / 240）。

平均加速比（括号内为中位数）：

| 问题 | 1 核 | 2 核 | 3 核 | 4 核 | 5 核 |
|---|---|---|---|---|---|
| 问题1 | 1.005 | 1.837 (1.96) | 2.573 (2.82) | 3.221 (3.46) | 3.762 (4.08) |
| 问题2 | 1.016 | 1.993 (1.99) | 2.866 (2.95) | 3.691 (3.83) | 4.322 (4.52) |
| 问题3 | 1.024 | 2.008 (2.00) | 2.903 (2.95) | 3.764 (3.87) | 4.430 (4.66) |
...
| 问题3 | 1.21 | 1.24 | 1.29 | 1.35 | 1.45 |
  ...
  1 核时就有约 1.22 倍的差距，来自单核按序发射下 M、V 流水线无法完全重叠，与多核调度无关。问题2 从 1 核到 5 核只增加到 1.50；5 核时有 28 个用例在下界的 1.1 倍以内，42 个在 1.25 倍以内。起作用的下界项绝大多数是 $M/N$（68 个用例）或 $V/N$（27 个），说明这些图在 5 核下仍受计算量限制。离下界最远的是强耦合小图 case_071（3.59 倍）、case_069（3.25 倍）、case_064（3.20 倍），它们的单核 makespan 只有 1.9 万–2.5 万周期，跨核依赖的固定时延和拷贝相对计算量太大。
- **只读 Cache**：相对无 L2 平均 1.007–1.030 倍，核数越多收益越大。5 核时 50 个用例提升超过 1%，最高的是 case_073（5 核 1.617 倍）、case_044（1 核 1.432 倍）和 case_083（3 核 1.273 倍）。单核基线有 spill 的用例收益更大（5 核平均 1.042 倍，无 spill 的 1.022 倍），与 6.3 节“spill 回读命中”的分析一致。有 2 个组合问题3 比问题2 慢 0.01%–0.1%：两者用的是同一个方案，是 Cache 改变执行时序带来的微小反常。
- **兜底比较**触发 89 次：31 次换成上一核数的方案，58 次是问题3 换成问题2 的方案。平均挽回 2.9%，最多 13.4%（case_040 问题3、3 核）。1500 个组合的 makespan 全部随核数单调不增。
...
A题方案/
  ...
  results/                实验输出（plans/ 方案文件，eval/ 官方输出，summary.csv，table_p*.md，*.png）
